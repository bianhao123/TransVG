...
def train_one_epoch(args, model: torch.nn.Module, data_loader: Iterable, 
    ...
    for batch in metric_logger.log_every(data_loader, print_freq, header):
        img_data, text_data, target = batch # img_data: [mask:(bs, 640, 640), tensors:(bs, 3, 640, 640)]
        # text_data: [mask: (bs, 20), tensors: (bs. 20)]  target: (bs, 4)
        # copy to GPU
        ...
        # model forward
        output = model(img_data, text_data) # output: (bs, 4)
